...
export async function POST(request: Request) {
  try {
    ...
    for (const parent of parents) {
      try {
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                ...
                product_data: {
                  name: name,
                  description: `${feeType === 'league' ? 'League' : 'Tournament'} Fee - ${season}`,
                },
        ...
        const feeRecord = await prisma.league_fees.create({
          data: {
            id: `fee_${Date.now()}_${parent.id.slice(-6)}`,
            parentId: parent.id,
            amount: amount,
            status: 'pending',
    ...
    return NextResponse.json({
      success: true,
      message: `Created ${results.created} fees and sent ${results.sent} reminders`,
      results
    })

  } catch (error: any) {
    console.error('Create and send fees error:', error)
    return NextResponse.json(
      { error: 'Failed to create and send fees', details: error.message },
      { status: 500 }
    )
  }
}
